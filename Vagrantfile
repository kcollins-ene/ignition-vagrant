...
if [ ! -f /vagrant/Ignition-7.9.3-linux-x64-installer.run ] || [ "`md5sum /vagrant/Ignition-7.9.3-linux-x64-installer.run | cut -c 1-32`" != "a45be3291cb852246a62602a911ea241" ]; then
  echo "Downloading Ignition 7.9.3"
  wget -q https://s3.amazonaws.com/files.inductiveautomation.com/release/ia/build7.9.3/20170602-1004/Ignition-7.9.3-linux-x64-installer.run -O /vagrant/Ignition-7.9.3-linux-x64-installer.run >> install.log
else
  echo "Existing Installer Detected, Skipping Download"
fi
echo "Installing Ignition 7.9.3"
chmod a+x /vagrant/Ignition-7.9.3-linux-x64-installer.run
sudo /vagrant/Ignition-7.9.3-linux-x64-installer.run --unattendedmodeui none --mode unattended --prefix /usr/local/share/ignition >> install.log
